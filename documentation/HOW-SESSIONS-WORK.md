...
**Sessions in Onyx work like a secure personal vault system with three layers of protection.** When you log in, the system creates a unique digital key (called a JWT token) that identifies you, stores it temporarily in fast memory (Redis cache) for quick access, and uses it to ensure every piece of data you create or view is automatically tagged with your user ID and stored permanently in the database (PostgreSQL) with complete isolation from other users. Each time you send a message, upload a file, or create a chat, the system follows a strict four-step security process: (1) it validates your identity token to ensure you're logged in, (2) checks that you own the data you're accessing through ownership verification, (3) creates database records linked to your user ID so your data is permanently associated with your account, and (4) stores temporary session information in cache for fast retrieval while maintaining complete data separation between users at multiple layers. The result is a secure system where your conversations, files, and projects are completely private and isolated, with database queries automatically filtering by user_id, API endpoints validating ownership before returning data, and Redis cache storing sessions with tenant isolation to prevent any cross-user data access.

### **For Technical Users:**

**Sessions in Onyx implement a multi-layered authentication and authorization architecture using JWT tokens, PostgreSQL with Row-Level Security, and Redis cache for session state management.** The authentication flow follows this sequence: (1) User authenticates → FastAPI validates credentials against PostgreSQL `user` table → JWT token generated with `user_id` and `tenant_id` claims → Token stored in Redis with key pattern `auth:session:{token_hash}` and TTL of 24 hours → Token returned to client as HTTP-only cookie or Authorization header; (2) User makes API request → NGINX extracts token from request headers → FastAPI middleware validates token signature and expiration using secret key → Extracts `user_id` from token claims → Sets `current_user` context in request scope for downstream operations. **Data operations enforce isolation through multiple layers:** (3) All database queries automatically filter by `user_id` via SQLAlchemy session scoping and tenant context → PostgreSQL RLS (Row-Level Security) policies enforce additional isolation at the database level → API endpoints verify ownership before returning data using dependency injection (`Depends(current_user)`) → Results cached in Redis with tenant-scoped keys using `TenantAwareRedisStrategy` for namespace isolation. **Session persistence and management:** (4) Chat sessions stored in `chat_session` table with `user_id` foreign key constraint → Messages linked via `chat_session_id` foreign key → Active sessions cached in Redis with pattern `session:{session_id}` for sub-second retrieval → Session expiration handled by Redis TTL (Time-To-Live) and background cleanup jobs → All operations are transactional and ACID-compliant using PostgreSQL transactions for data integrity, ensuring complete isolation and preventing race conditions or data leakage between users.
